...
- **主工作流（Reusable Workflow）**：完整的阶段流水线，调用方一行引入  
  `uses: org/openCI/.github/workflows/pr.yml@v3`
...
**主工作流清单**(`.github/workflows/`):`agent` `issue` `pr` `ci` `stg` `prd` `prd-observe` `security-schedule` `docs` `deps` `release`。
...
# 校验:.github/workflows/security.yml(每个 PR 触发)
# ─────────────────────────────────────────────────────────────────────────────
deps:
  # ── GitHub 官方(全部已验证) ──────────────────────────────────────────
...
workflows:
  ...
  - id: issue
    ...
    description: Issue agent orchestrator(lifecycle / maintenance / ingest)
    inputs:
      mode: { type: string, default: lifecycle }
    secrets:
      anthropic-api-key: { required: false }

  - id: community
    path: .github/workflows/issue.yml
    description: Issue lifecycle comments and agent-planned follow-up actions
    secrets: {}
...
OpenCI 在 `.github/ISSUE_TEMPLATE/` 提供 YAML form 模板范例(`bug-report.yml` / `feature-request.yml` / `question.yml` / `security-report.yml`)。YAML form 的结构化字段(必填 + 下拉)先进入确定性 ingest 阶段,再与仓库上下文、Agent workspace、MCP task 元数据一起交给 issue agent 规划后续动作。
...
**触发**:`on-issue.yml` 监听 `issues` / `issue_comment` / `schedule` / `repository_dispatch` / `workflow_dispatch`,然后调用 `.github/workflows/issue.yml`。

**模式**:

| Mode | 行为 |
| --- | --- |
| `lifecycle` | issue 与 issue comment 的主流程。 |
| `maintenance` | stale / close / lock 等确定性维护。 |
| `ingest` | Linear / Sentry 等外部 issue-like 事件进入统一 agent 流。 |

**四阶段流水线**:

| Stage | Job | 行为 |
| --- | --- | --- |
| 1 | `ingest` | 使用成熟 issue action 解析 issue form、按字段打基础 label、收集 duplicate candidates,输出 `ingest.json`。 |
| 2 | `enrich` | 合并 `.github/agent/shared` 与 `.github/agent/issue` 的 `AGENTS.md` / skills,再加入评论、相关 issue、外部 tracker、MCP 与 env metadata。 |
| 3 | `agent` | 调用 `_common/claude-harness`,Agent 只输出 `issue-action-plan/v1` JSON,不直接写 GitHub。 |
| 4 | `execute` | 校验 action plan schema、skill allowlist、作者权限和 idempotency 后执行 GitHub mutation,并写审计评论。 |

### 6.3 Agent Workspace

Issue agent 使用共享 + 专属两层 workspace:
...
每个 `context/` 目录只允许一个入口文件 `AGENTS.md`。Enrich 阶段把 shared 与 issue 专属 context/skills 合并成 `agent-workspace/`,效果等价于为 issue 管理单独启动一个带工具、记忆、MCP、环境元数据的 Claude CLI 实例。

### 6.4 Agent Action Plan
...
允许的 skill 由 `.github/agent/shared/skills/*.md` 和 `.github/agent/issue/skills/*.md` 决定。Executor 默认支持 `add_label` / `remove_label` / `set_priority` / `assign_issue` / `add_comment` / `close_issue` / `reopen_issue` / `mark_duplicate` / `create_branch` / `link_linear` / `dispatch_mcp_task` / `schedule_followup` / `notify` / `escalate`。

外部执行约定:`link_linear` 使用 `linear-token` 通过 Linear GraphQL 写入评论;`dispatch_mcp_task` 只允许 `runtime/mcp-tasks.json` 中声明的任务,并通过受控 `repository_dispatch` 发出;`schedule_followup` 写入 issue marker 与 `followup:scheduled` label,由 maintenance 扫描到期事项;`notify` 使用 `slack-webhook-url` 发送 Slack-compatible webhook。

安全边界:Agent 只规划,Executor 执行。外部 contributor 评论不能触发高风险动作(`close_issue` / `reopen_issue` / `create_branch` / `dispatch_mcp_task`)。安全相关 issue 不公开复述敏感细节,优先 escalate。

### 6.5 维护清理

`maintenance` 模式使用 `actions/stale` 和 `dessant/lock-threads` 做确定性清理,并扫描 `followup:scheduled` issue marker,到期后评论提醒、移除 `followup:scheduled`、添加 `needs-followup`。**Inputs**:`issue-stale-days`(60)/ `issue-close-days`(14)/ `pr-stale-days`(30)/ `pr-close-days`(7)/ `lock-after-days`(30)。豁免 label:`pinned,security,help-wanted,priority:p0,priority:p1`(issue),`pinned,security,wip`(PR)。
...
jobs:
  quality:
    uses: your-org/openCI/.github/workflows/pr.yml@v3
    with:
...
jobs:
  build:
    ...
    uses: your-org/openCI/.github/workflows/ci.yml@v3
    with:
      image-name: my-app
      run-migration: true
    secrets:
  ...
  deploy-stg:
    needs: build
    uses: your-org/openCI/.github/workflows/deploy.yml@v3
    with:
...
`.github/ISSUE_TEMPLATE/` 目录提供结构化模板,字段化 + 必填校验,后续 issue ingest 与 agent enrichment 能直接读字段。
...
| # | 实施内容 | 验证方式 | 来源 |
|---|---------|---------|------|
| 22 | `on-issue.yml` / `issue.yml` | issue agent lifecycle / maintenance / ingest 触发验证 | 6 章 |
...
**基础设施准备**(每个 e2e workflow 内部完成):前置 `setup-k8s` job 用 `helm/kind-action` 启 kind 集群,kind export kubeconfig 后 base64 编码 + `::add-mask::` 后塞到 step output;后续 `test-stg` job `needs: setup-k8s`,`uses: ./.github/workflows/deploy.yml`,通过 `secrets.kubeconfig-stg: ${{ needs.setup-k8s.outputs.kubeconfig }}` 传递。
