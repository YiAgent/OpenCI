...
> 后续章节里所有标注为"Secrets Preflight"的代码块都遵循这个完整 `jobs.preflight` 结构,为节省篇幅省略了 `jobs:` 顶层包裹。
...
### 5.5 prd.yml(v1.1 强化,v1.3 扩展,v1.7 整合)
...
jobs:
  deploy-k8s:
    ...
    environment:
      name: production
      url: https://app.example.com    # 部署完成后展示
    # ...
  run-migration:
    needs: deploy-k8s
...
jobs:
  ...
  notify-deployed:  { timeout-minutes: 5 }
  notify-observability: { timeout-minutes: 5 }   # 调用 notify-deploy composite
```

**Job 顺序**(v1.7 整合 Sentry/PostHog/Datadog 等可观测性 push 到末尾的 `notify-observability` job):
...
- **可观测性事件推送统一收口**:Sentry release / PostHog deploy event / Datadog deployment event / LangSmith tag / Axiom log 全部由 `notify-deploy` composite 在 `notify-observability` job 一次性扇出。**不再作为独立 jobs 出现**(v1.6 的双重定义已修复)。
...
**已知技术债(P1,标注为 v1.7 的待清理项)**:

当前 `observe-window` 用 `sleep $WAIT` 阻塞 runner,30 分钟观察期 × 每月 ~30 次部署 = 每月 ~15 小时 runner 时间浪费。**P1 迁移方案**(在 v1.8 实施):
...
结论:采用方案 A,P1 在 v1.8 实施。
```

**v1.7 临时缓解**:`pre-check` job 的 `timeout-minutes: 45` 已含观察窗口预算,且 `concurrency.cancel-in-progress: false` 避免 runner 浪费在被取消的 run 上。
...
### 5.7 Concurrency 在 Reusable Workflow 中的语义(v1.7 新增)
...
**集成点**(v1.7 修正):**不再作为独立 jobs**,统一通过 7.8.1 节的 `notify-deploy` composite 在 stg/prd 末尾的 `notify-observability` job 中按 `enable-sentry` 开关启用。原 stg.yml 的 `notify-deployed` job 仅负责 Slack 通知,Sentry release 已由 `notify-observability` 接管。
...
**集成点**(v1.7 修正):**不再作为独立 jobs**,统一通过 7.8.1 节的 `notify-deploy` composite 调用。posthog-event 原子由 composite 在 stg/prd 末尾的 `notify-observability` job 中按 `enable-posthog` 开关启用。
...
> **关于 outputs 的语法注意**:消费方调用 reusable workflow 时,**job 不需要也不能在自己声明 `outputs:` 段**——reusable workflow 的 outputs 由它自身的 `on.workflow_call.outputs` 声明,调用方通过 `needs.<job-id>.outputs.<key>` 直接读取。下面示例已修正(v1.6 错误版本会在 GitHub Actions 中报 schema 错误)。
...
**CODEOWNERS 与 auto-assign 分工(v1.7 明确)**:
...
**OpenCI v1.7 决策:推荐 Renovate,而非 Dependabot**。原因:
...
Claude Code 按以下顺序实施,每阶段可独立验证。v1.7 把 16 章 Aicert 对比中的 P0 项(原 v1.6 全打包到 P5)按其本身优先级散到对应阶段。
...
## 附录 D:SHA 一致性验证脚本(v1.7 新增)

`.github/scripts/verify-sha-consistency.sh` 是 P0 必备 CI job,确保仓库内所有 `uses:` 引用的 SHA 与 `manifest.yml` 一致。

**职责**:

1. 扫描所有 `.github/workflows/*.yml` 与 `actions/**/*.yml` 中的 `uses: <action>@<SHA>` 行
2. 对每条引用,在 `manifest.yml` 的 `deps:` 段查找 `<action>` key
3. 比对实际 SHA 与 manifest SHA,不一致则 `exit 1`
4. 同时检查:
   - 没有 `@v\d+` / `@main` / `@master` 形式的 tag/branch 引用(全部必须是 40 位 SHA)
   - 没有 `<action>` 出现在 `manifest-pending.yml` 中却被引用(未验证 SHA 不允许进主仓库)
   - 没有 `manifest.yml deprecated` 段中的 action 仍在被使用
...
PENDING=manifest-pending.yml

# 1. 扫描所有 uses
REFS=$(grep -rhE 'uses:\s*[a-zA-Z0-9._/-]+@[a-f0-9]{40}' \
        .github/workflows/ actions/ \
        | sed -E 's/.*uses:\s*([^@]+)@([a-f0-9]{40}).*/\1 \2/' \
        | sort -u)

FAIL=0
while read action sha; do
  # 2. 主 manifest 查找
  EXPECTED=$(yq ".deps.\"$action\"" $MANIFEST 2>/dev/null || echo null)

  if [ "$EXPECTED" = "null" ]; then
    # 在 pending 里?
    if yq ".deps.\"$action\"" $PENDING >/dev/null 2>&1; then
      echo "::error::action $action is in manifest-pending.yml but already used. Migrate after verification."
      FAIL=1
      continue
    fi
    # 在 deprecated 里?
    if grep -q "$action" docs/SPEC.md; then
      echo "::error::action $action is deprecated. See SPEC.md Appendix B."
      FAIL=1
      continue
    fi
    echo "::error::action $action used in repo but not in $MANIFEST"
    FAIL=1
    continue
  fi
  ...
  if [ "$EXPECTED" != "$sha" ]; then
    echo "::error::SHA mismatch for $action: file=$sha manifest=$EXPECTED"
    FAIL=1
  fi
done <<< "$REFS"

# 4. 检查非 SHA 引用
NON_SHA=$(grep -rhE 'uses:\s*[a-zA-Z0-9._/-]+@[^a-f0-9]' \
          .github/workflows/ actions/ \
          | grep -vE 'uses:\s*\./' \
          | grep -vE '@[a-f0-9]{40}' || true)

if [ -n "$NON_SHA" ]; then
  echo "::error::non-SHA uses found:"
  echo "$NON_SHA"
  FAIL=1
fi
...
**触发**:每个 PR + push 到 main 都跑(挂在 `pr.yml` 与 `ci.yml` 的 preflight 之后)。

---

## 附录 E:Dependabot 备选配置(v1.7 新增)

> OpenCI 默认推荐 Renovate(见 17.4 节)。本附录仅为已经在用 Dependabot 的团队提供过渡配置。**不支持 SHA pinning,需配合 `pin-github-action` CI job**。
...
- 不会自动 pin 到 commit SHA(只更新版本号)。需在 `.github/workflows/pin-action-shas.yml` 加一个独立 job,Dependabot 提 PR 后自动跑 `npx pin-github-action` 把版本 tag 改成 SHA。
- 不支持跨 manager 分组(GitHub Actions + npm 不能合并到一个 PR)。
- patch 自动合并需依赖 `dependabot/auto-merge` 第三方 action。

如果团队对 Dependabot 没有强依赖,**建议直接迁移到 Renovate**。
